...
- (void)dealloc {
    [_adapter release];
	[_lastErrorMsg release];
	[_lastResultDate release];
...
- (void)didFailToReceiveAd:(AdamAdView *)adView error:(NSError *)error {
    NSString * errorMsg = [[NSString stringWithUTF8String:[[error domain] cStringUsingEncoding:NSUTF8StringEncoding]] retain];
    if( _lastErrorMsg)
        [_lastErrorMsg release];
